...
function actualizarCirculo(porcentaje) {
    ...
    const circleFill = circle.querySelector('.fill');
    const circleText = circle.querySelector('span');
    const input = document.getElementById('porcentajeInput');

    // Asegurarnos de que el porcentaje esté dentro del rango de 0 a 100
    porcentaje = Math.min(100, Math.max(0, porcentaje));

    // Actualizar el círculo y el texto
    circleFill.style.transform = `rotate(${(porcentaje / 100) * 360}deg)`;
